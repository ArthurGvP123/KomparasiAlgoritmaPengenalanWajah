...
# Pipeline terpadu (varian cosine):
#  - SELALU membuat pairs otomatis dari embeddings
#  - Evaluasi verifikasi (ROC/AUC/EER/TAR) + confusion matrix pada threshold tetap
#  - Hitung Accuracy / Precision / Recall / F1
#  - Simpan:
#      * Semua output per algoritma ke: eval_verif_cosine/<algo>/
...
#      * CSV agregat semua algoritma: eval_verif_cosine/metrics_all.csv
#
...
# =============== Util path & naming ===============
EVAL_ROOT_NAME = "eval_verif_cosine"

def find_eval_root(outdir: Path) -> Path:
    """Cari folder 'eval_verif_cosine' terdekat di jalur outdir; fallback ke outdir."""
...
def eval_verification_pipeline(
    ...
    """
    Jalankan end-to-end untuk satu file .npz.
    - Semua output disimpan di eval_verif_cosine/<algo>/ dengan nama file bertag threshold.
    - CSV agregat semua algoritma: eval_verif_cosine/metrics_all.csv
    """
    ...
    scores = np.array([float(np.dot(EMB[k1], EMB[k2])) for k1, k2, _ in fixed_pairs], dtype=float)
    ...
    idx_1e4 = np.clip(idx_1e4, 0, len(thr) - 1)
    thr_1e4 = float(thr[idx_1e4]) if len(thr) > 0 else 0.0

    # Confusion matrix dengan threshold tetap
    thr_used = float(fixed_thr)
    ...
    # Kolom DIKURANGI: 'embeds', 'pairs_auto', 'pairs_fixed', 'roc_png' Dihapus dari CSV agregat.
    metrics_all_csv_fieldnames = [
        ...
        # (metadata path TIDAK dimasukkan lagi ke CSV agregat)
    ]
...
def main():
    ap = argparse.ArgumentParser(
        description="Evaluasi verifikasi + metrik (gabungan 03a+04+05, cosine) dengan output per-algoritma & CSV agregat."
    )
    ...
    ap.add_argument("--fixed-thr", type=float, default=0.6,
                    help="Threshold tetap untuk TP/TN/FP/FN (default: 0.6).")
